...
from fcos_core.utils.poly_nms.poly_nms import poly_nms
...
def handle_ratio_prediction(prediction):
    ...
    h_idx = np.where(ratios > 0.8)[0]  #评测标准
    h = hboxes[h_idx]
...
def do_dota_evaluation(
    ...
    for image_id, prediction in tqdm( enumerate(predictions) ):
        ...
        #容易将缩小0.5的P0024__0.5__0___0.png标签拿取错误
        #img_name = os.path.basename( img_info["file_name"] ).split( "." )[0]
        #修改
        img_name = os.path.basename(img_info["file_name"]).replace(".png","")
