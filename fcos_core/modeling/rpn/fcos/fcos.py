...
import numpy as np
import os
feature_path="/data/maq/PycharmProjects/FCOS_to_glid_vertex/demo/feature_tmp/"
...
class FCOSModule(torch.nn.Module):
    ...
    def compute_locations_per_level(self, h, w, stride, device):
        shifts_x = torch.arange(
            0, w * stride, step=stride,  #debug  生成较密的中心点位置 stride-->stride/4
        ...
        locations = torch.stack((shift_x, shift_y), dim=1) + (stride //2 ) #debug 对坐标进行移动
        return locations
...
#可视化特征图
def visualize_featuremap(feature):
    # use sigmod to [0,1]
    feature = 1.0 / (1 + np.exp(-1 * feature))
    # to [0,255]
    feature = np.round(feature * 255)
    feature=np.expand_dims(feature,axis=2)
    feature_img=np.concatenate([feature,feature,feature],axis=2)
    feature_img = cv.resize(feature_img, (1024, 1024), interpolation=cv.INTER_LINEAR)
    return feature_img