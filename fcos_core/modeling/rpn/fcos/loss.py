...
class FCOSLossComputation(object):
    ...
    def get_sample_region(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1.0):
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            stride = strides[level] * radius    #fixme 是否此处也要修改为生成中心点的一
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest):
        ...
        for im_i in range(len(targets)):
            ...
            # # torch.Size([21824, 9, 4]) <class 'torch.Tensor'>
            #todo  删除中心不在标注框内的点
            if self.center_sampling_radius > 0:   #todo 是改进中心点的
                is_in_boxes = self.get_sample_region(   #debug
                    bboxes,
                    self.fpn_strides,
    ...
    def __call__(self, locations, box_cls, box_regression,centerness,fix_regression,ratio_regression,targets):
        ...
        for l in range(len(labels)):
            ...
            # print("###box_cls_flatten[l]:", box_cls_flatten[l].shape, type(box_cls_flatten[l]))
            # print("###labels_flatten[l]:", labels_flatten[l].shape, type(labels_flatten[l]))
            #
        ...
        # print("###box_cls_flatten:", len(box_cls_flatten))
        # print("###labels_flatten:", len(labels_flatten))
        # print("###box_regression_flatten:", len(box_regression_flatten))
        # print("###reg_targets_flatten:", len(reg_targets_flatten))
        #todo 选择正例坐标   ################
        ...
        if pos_inds.numel() > 0:
            centerness_targets = self.compute_centerness_targets(reg_targets_flatten)  #fixme 暂时先屏蔽
